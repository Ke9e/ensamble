...
function drawRulers() {
  ...
  // Offset del canvas dentro del wrap
...
function render() {
  ...
  if (acousticMode && acousticNodes.length) {
    ...
    for (const n of acousticNodes) {
      ...
      if (hasType) {
        ...
        const crvCount = Number.isFinite(n._audioVoiceCount) ? n._audioVoiceCount : fallbackCount;
  ...
  document.getElementById('infoTag').textContent =
    `${curves.length} conexiones  -  ${nodes.length} nodos  -  ${types.length} tipo${types.length!==1?'s':''}`;
...
`;
    for (const x of gridAxes.colX) {
      svg += `  <line x1="${x.toFixed(2)}" y1="0" x2="${x.toFixed(2)}" y2="${H}"/>
`;
    }
    for (const y of gridAxes.rowY) {
      svg += `  <line x1="0" y1="${y.toFixed(2)}" x2="${W}" y2="${y.toFixed(2)}"/>
`;
    }
    svg += `</g>
`;
...
`;
  for (const c of curves) {
    ...
    const d   = 'M' + c.pts.map(p => `${p.x.toFixed(2)},${p.y.toFixed(2)}`).join('L');
...
`;
  for (const n of nodes) {
    svg += `  <circle cx="${n.x.toFixed(2)}" cy="${n.y.toFixed(2)}" r="3" fill="rgba(200,210,230,0.85)"/>
`;
  }
  svg += `</g>
`;
